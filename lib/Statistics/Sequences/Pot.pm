...
#-----------------------------------------------
    ...
    if (!$n) { # State $state did not occur within the data-set for pot-testing
        ...
        $self->_expound(_calc($n, $m, $self->{'scale'}, \@indices), $args);
    }
...
The range of observations over which Pot was assessed, simply being the product of the C<scale> and number of observations, divided by the number of states.
...
The probability associated with the absolute value of the I<z>-value.
...
Limitations of the actual Pot statistic may be considered to be its newness, although it has been published in a peer-reviewed journal. Some experimentation might yet be required to find an optimal C<scale>.
